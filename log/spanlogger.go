// https://github.com/jaegertracing/jaeger/tree/master/examples/hotrod/pkg/log
// 主要增加Warn/Debug Level, 把写入span log的部分稍作修改
...
func (sl spanLogger) Info(msg string, fields ...zapcore.Field) {
	if r := sl.logger.Check(zapcore.InfoLevel, msg); r != nil {
...
func (sl spanLogger) Warn(msg string, fields ...zapcore.Field) {
	if r := sl.logger.Check(zapcore.WarnLevel, msg); r != nil {
...
func (sl spanLogger) Error(msg string, fields ...zapcore.Field) {
	if r := sl.logger.Check(zapcore.ErrorLevel, msg); r != nil {
...
func (sl spanLogger) logToSpan(level string, msg string, fields ...zapcore.Field) {
	...
	fa = append(fa, log.String("message", msg))
...
func (fa *fieldAdapter) AddTime(key string, value time.Time) {
	// change to standard RFC3339 format
	*fa = append(*fa, log.String(key, value.Format(time.RFC3339Nano)))
}
